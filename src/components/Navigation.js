...
function Navigation({subhead}) {
    ...
    return (
        <div className={`Navigation ${scrolled?"fixed_Nav_home":""}`}>
            <nav className={`boxwidth ${path==="/" && !scrolled?"nav_home":""}`}>
                <div className={`logo ${path==="/" && !scrolled?"logo_home":""}`}>
                    <h1>Showcase</h1>
                    {subhead && <h3>{subhead}</h3>}
                </div>
                <div className={`right ${scrolled?"right_scrolled":""}`}>
                    {menuOpen && <div onClick={()=>setMenuOpen(!menuOpen)} className="clickToExit"/>}
                    ...
                    <ul className={`hamburger_menu ${menuOpen? "menu_open": "menu_close"}`}>
